...
from fields import Field, ForeignKey, CharField
...
class BaseModel:
    ...
    #     Handles `null`, `blank`, and `default` parameters for each field.
    #     """
    #     if not kwargs:
    ...
    def insert(cls, verbose=True, **kwargs):
        ...
        try:
            ...
            # for name, field in model_fields.items():
            #     if getattr(field, "unique", False):  # Check if the field is marked as unique
            #         sql = f"SELECT COUNT(*) FROM {cls.table_name or cls.__name__.lower()} WHERE {name} = ?"
            #         with connect_to_database() as conn:
            #             cursor = conn.cursor()
            #             cursor.execute(sql, (kwargs[name],))
            #             result = cursor.fetchone()
            #             if result and result[0] > 0:
            #                 raise ValueError(f"Field '{name}' must be unique. Duplicate value '{kwargs[name]}' found.")
    ...
    #
    #         if not all_columns:
    #             raise ValueError("No valid columns available for bulk insert.")
    #
    #         required_fields = {
    #             name for name, field in model_fields.items()
    #             if not field.null and getattr(field, "default", None) is None and name != "id"
    ...
    #
    #             # Validate the record
    #             cls.validate_fields({col: val for col, val in zip(all_columns, row_values)})
    #
    #             # Append validated record as tuple
    ...
    #
    #         # Step 4: Build and execute the bulk insert SQL query
    #         sql = f"INSERT INTO {cls.table_name or cls.__name__.lower()} ({columns}) VALUES ({placeholders})"
    #
    #         # Execute the query for all records at once
    #         with connect_to_database() as conn:
